...
class ConformerTrainerLOOCV:
    ...
    def _create_model(self, use_multiscale=False):
        """
        创建 Conformer 模型
        ...
        消融实验顺序：
            步骤1: use_multiscale=False (基线测试)
            步骤3: use_multiscale=True (多尺度卷积测试)
        """
        model = Conformer(
            emb_size=40, 
            depth=6, 
            ...
            dropout=self.dropout            # 根据use_multiscale动态设置的dropout率
        )
    ...
    def train_patient_loocv(self, patient_id):
        ...
        for fold_idx, test_seizure_id in enumerate(seizure_indices):
            ...
            model = self._create_model(use_multiscale=self.use_multiscale)
